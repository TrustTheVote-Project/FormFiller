...
module FormFiller
  ...
  class Signer
    attr_accessor :form, :signature, :document, :signature_field_name, :signature_position
    def initialize(args)
      @form = args[:form]
...
#      @signature_position = args[:sig_pos] || form.field_position(signature_field_name)
      ...
      if pos
        ...
        @signature_position = form.bbox(signature_field_name)
      end
    end    
    
    def sign
      bbox = signature_position
      signature.scaleToFit(bbox.width, bbox.height)
      signature.setFixedPosition(bbox.left, bbox.bottom)
      form.remove_field(signature_field_name) if signature_field_name
